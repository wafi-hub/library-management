...
cout << "Enter initial deposit amount (TK 50 minimum): ";
cin >> initial_deposit;

if (initial_deposit < 50) {
    cout << "Initial deposit must be at least TK 50.\n";
    return;
}
...
cout << "Balance: TK" << fixed << setprecision(2) << student_balance[index] << endl;
}
...
cout << "New balance: TK" << fixed << setprecision(2) << student_balance[index] << endl;
}
...
for (int i = 0; i < student_count; i++) {
    cout << student_roll[i]<< " - " << student_name[i] << " - Balance: TK" << fixed << setprecision(2) << student_balance[i] << endl;
}
}
